...
class Mysql (Check):
    ...
    def check (self):
        if len(self.users) < 1:
            return (OFFLINE_TEAM_ID, "No users available")
    ...
    def check (self):
        ...
        for criterion in self.criteria:
            try:
                ...
                target_host = f"manehatten.ravencrest.local,1433" # TODO hardcoded!
                db_name = "db"
...
class Workstation_windows (Check):
    ...
    def __init__(self, check: Service) -> None:
        ...
        self.users = []
        for user in users:
            if user.username in LOCAL_ADMINS:
                self.users.append((user.username, user.password))
